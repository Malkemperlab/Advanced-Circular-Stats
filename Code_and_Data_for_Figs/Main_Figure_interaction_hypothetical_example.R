...
specs_Fig1 <- c("Main_Figure_interaction_examples_Oct2025.pdf","Power","Sample size",1)
...
for (i in 1:nrow(specs)) {
 ...
 # FigurexD <-list_all[[i]][[4]]
  ...
  pdf(name_fig, width=6, height=6)
  
  layout_matrix <- matrix(seq(1:(4*2)), nrow = 4, byrow = TRUE)
  
  layout_widths <- c(1.1, 1)  # First column wider
  ...
  legend_1= function(){
    ...
    legend("bottomleft", legend=c("Rayleigh","HR","CircMLE","MANOVA (intercept)",
                                  "MANOVA interaction  (intercept)",
                                  "LM (intercept)","LM interaction (intercept)"),
           
           
           col=colors_plot[c(name_vector_col)],
           
           cex = 1.3, lty=c(1:2,6,3,5,1,2),pch = 1,bty="n",
           lwd = 0.6,y.intersp = yesp,seg.len = seglen,xjust = 0, x.intersp=0.1 )}
  
   
  legend_4= function(){
    name_vector_col<-c("MANOVA_linear_group_groupP_col",
                       "LM_lin_group_P_group_col")
    
    legend("bottomleft", legend=c("MANOVA interaction (interaction)",
                                  "LM interaction (interaction)" ),
           col=colors_plot[name_vector_col],cex = 1.3, lty=c(1,3),pch = 1,bty="n",
           lwd = 0.6,y.intersp = yesp,seg.len = seglen,xjust = 0, x.intersp=0.1 )}
...
#  lines(FigurexA$nseq,FigurexA$MANOVA_linear_group_int_interceptP,lty=6,col=colors_plot["MANOVA_random_linear_group_interceptP_col"],type = "b",pch = 1,lwd = 0.6)
...
#  lines(FigurexB$nseq,FigurexB$MANOVA_random_intercept,lty=4,col=colors_plot["MANOVA_random_intercept_col"],type = "b",pch = 1,lwd = 0.6)
 ...
 # lines(FigurexB$nseq,FigurexB$MANOVA_linear_group_int_interceptP,lty=6,col=colors_plot["MANOVA_random_linear_group_interceptP_col"],type = "b",pch = 1,lwd = 0.6)
 ...
 # lines(FigurexC$nseq,FigurexC$MANOVA_linear_group_int_interceptP,lty=6,col=colors_plot["MANOVA_random_linear_group_interceptP_col"],type = "b",pch = 1,lwd = 0.6)
