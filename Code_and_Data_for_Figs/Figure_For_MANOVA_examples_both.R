...
list_fig1 <- list(Figure1A, Figure1B,Figure1C,Figure1D)
list_fig2 <- list(Figure2A, Figure2B,Figure2C,Figure2D)
list_fig3 <- list(Figure3A, Figure3B,Figure3C,Figure3D)
list_fig4 <- list(Figure4A, Figure4B,Figure4C,Figure4D)

list_all<-list(list_fig1,list_fig2,list_fig3,list_fig4)
...
list_type<-list(type[1:4],type[5:8],type2[1:4],type2[5:8])
...
ylab_vec3<-c("Type 1 error","Type 1 error","Type 1 error",
             "Power","Power","Type 1 error",
             "Power","Power","Type 1 error",
             "Power","Power","Power")

ylab_vec4<-rep("Power",12)


specs_Fig1 <- list("Figure_hypothetical_examples_1_Oct2024.pdf",ylab_vec1,"Sample size",1)
specs_Fig2 <- list("Figure_hypothetical_examples_2_Oct2024.pdf",ylab_vec2,"Sample size",1)
specs_Fig3 <- list("Figure_hypothetical_examples_k4ID_1_Oct2024.pdf",ylab_vec3,"Sample size",1)
specs_Fig4 <- list("Figure_hypothetical_examples_k4ID_2_Oct2024.pdf",ylab_vec4,"Sample size",1)


specs <- list(specs_Fig1,specs_Fig2,specs_Fig3,specs_Fig4)
...
layout_matrix <- matrix(seq(1:(5*3)), nrow = 5, byrow = TRUE)
...
legend_1= function(){
  ...
  name_vector_col<-c("Rayleigh_col","HR_col", "Model_comp_col","MANOVA_intercept_col","MANOVA_random_intercept_col" , 
                     "MANOVA_linear_interceptP_col","MANOVA_random_linear_interceptP_col",
  ...
  legend("bottomleft", legend=c("Rayleigh","HR","CircMLE","MANOVA (intercept)","simMANOVA (intercept)" , 
                                                   "MANOVA linear (intercept)","simMANOVA linear (intercept)",
                                                   "MANOVA linear & group (intercept)","simMANOVA linear & group (intercept)",
                                                   "LM (intercept)","LM linear (intercept)","LM linear & group (intercept)"),
...
legend_2= function(){
  ...
  name_vector_col<-c("MANOVA_linear_covariateP_col","MANOVA_random_linear_covariateP_col",
                     "MANOVA_linear_group_covariateP_col","MANOVA_random_linear_group_covariateP_col",
                     "LM_lin_P_lin_col","LM_lin_group_P_lin_col") 
  
  
  legend("bottomleft", legend=c("MANOVA linear (linear)","simMANOVA linear (linear)",
                                "MANOVA linear & group (linear)","simMANOVA linear & group (linear)",
                                                  "LM linear (linear)","LM linear & group (linear)"),

                            col=colors_plot[c(name_vector_col)],
         cex = 1.3, lty=c(1:6),pch = 1,bty="n",
                            lwd = 0.6,y.intersp = yesp,seg.len = seglen,xjust = 0, x.intersp=0.1 )}
...
legend_3= function(){
  ...
  legend("bottomleft", legend=c("Watson","G-test","MANOVA linear & group (group)","simMANOVA linear & group (group)",
                                                   "LM linear & group (group)" ),
...
legend_4= function(){
  ...
  legend("bottomleft", legend=c("Rayleigh (IDs averaged)","HR (IDs averaged)","CircMLE (IDs averaged)","MANOVA re (intercept)","simMANOVA re (intercept)" , 
                                "MANOVA re linear (intercept)","simMANOVA re linear (intercept)",
                                "MANOVA re linear & group (intercept)","simMANOVA re linear & group (intercept)",
                                "LMM (intercept)","LMM linear (intercept)","LMM linear & group (intercept)"),
         col=colors_plot[c(name_vector_col)],cex = 1.3, lty=c(1:2,6,3:6,1:5),pch = 1,bty="n",
         lwd = 0.6,y.intersp = yesp,seg.len = seglen,xjust = 0, x.intersp=0.1 )}
...
legend_5= function(){
  ...
  legend("bottomleft", legend=c("MANOVA re linear (linear)","simMANOVA re linear (linear)",
                                "MANOVA re linear & group (linear)","simMANOVA re linear & group (linear)",
                                "LMM linear (linear)","LMM linear & group (linear)"),
         col=colors_plot[c(name_vector_col)],cex = 1.3, lty=c(1:6),pch = 1,bty="n",
         lwd = 0.6,y.intersp = yesp,seg.len = seglen,xjust = 0, x.intersp=0.1 )}
...
legend_6= function(){
  ...
  legend("bottomleft", legend=c("Watson (IDs averaged)","G-test (IDs averaged)","MANOVA re linear & group (group)","simMANOVA re linear & group (group)",
                                "LMM linear & group (group)"),
         col=colors_plot[c(name_vector_col)],cex = 1.3, lty=c(1:5),pch = 1,bty="n",
         lwd = 0.6,y.intersp = yesp,seg.len = seglen,xjust = 0, x.intersp=0.1 )}
...
par(mar=c(2,9,0,0))
plot.new();if(i<3) {legend_1()} else {legend_4()}

par(mar=c(2,4,0,0))
plot.new();if(i<3) {legend_2()} else {legend_5()}

par(mar=c(2,4,0,0))
plot.new();if(i<3) {legend_3()} else {legend_6()}
...
par(mar=c(2,7,2,0))
plot(FigurexD$nseq,FigurexD$Rayleigh,col=colors_plot["Rayleigh_col"],type = 'b',lty=1,pch = 1,bty="n" ,
     ylab = ylab[10],xlab="Sample size per treat. comb.",xlim = c(xmin,xmax),ylim = c(0,ymax),lwd = 0.6,xpd=NA)
...
lines(FigurexD$nseq,FigurexD$MANOVA_intercept,col=colors_plot["MANOVA_intercept_col"],lty=3,type = "b",pch = 1,lwd = 0.6)
lines(FigurexD$nseq,FigurexD$MANOVA_random_intercept,lty=4,col=colors_plot["MANOVA_random_intercept_col"],type = "b",pch = 1,lwd = 0.6)
lines(FigurexD$nseq,FigurexD$MANOVA_linear_interceptP,col=colors_plot["MANOVA_linear_interceptP_col"],lty=5,type = "b",pch = 1,lwd = 0.6)
lines(FigurexD$nseq,FigurexD$MANOVA_random_linear_interceptP,lty=6,col=colors_plot["MANOVA_random_linear_interceptP_col"],type = "b",pch = 1,lwd = 0.6)
lines(FigurexD$nseq,FigurexD$MANOVA_linear_group_interceptP,col=colors_plot["MANOVA_linear_group_interceptP_col"],lty=1,type = "b",pch = 1,lwd = 0.6)
lines(FigurexD$nseq,FigurexD$MANOVA_random_linear_group_interceptP,lty=2,col=colors_plot["MANOVA_random_linear_group_interceptP_col"],type = "b",pch = 1,lwd = 0.6)

lines(FigurexD$nseq,FigurexD$LM_P_intercept,lty=3,col=colors_plot["LM_P_intercept_col"],type = "b",pch = 1,lwd = 0.6)
lines(FigurexD$nseq,FigurexD$LM_lin_P_intercept,lty=4,col=colors_plot["LM_lin_P_intercept_col"],type = "b",pch = 1,lwd = 0.6)
lines(FigurexD$nseq,FigurexD$LM_lin_group_P_intercept,lty=5,col=colors_plot["LM_lin_group_P_intercept_col"],type = "b",pch = 1,lwd = 0.6)


text(x = xmin-2.2, y = textn-1.1, labels = list_type[[i]][4], adj = 0, xpd = NA, srt = 90, font = 2,xpd=NA,cex=1.4)
...
xmin<-min(FigurexD$nseq)
par(mar=c(2,2,2,0))
plot(FigurexD$nseq,FigurexD$MANOVA_linear_covariateP,col=colors_plot["MANOVA_linear_covariateP_col"],type = 'b',lty=1,pch = 1,bty="n" ,
     ylab = ylab[11],xlab="Sample size per treat. comb.",xlim = c(xmin,xmax),ylim = c(0,ymax),lwd = 0.6,xpd=NA)
lines(FigurexD$nseq,FigurexD$MANOVA_random_linear_covariateP,col=colors_plot["MANOVA_random_linear_covariateP_col"],lty=2,type = "b",pch = 1,lwd = 0.6,)
lines(FigurexD$nseq,FigurexD$MANOVA_linear_group_covariateP,col=colors_plot["MANOVA_linear_group_covariateP_col"],lty=3,type = "b",pch = 1,lwd = 0.6)
lines(FigurexD$nseq,FigurexD$MANOVA_random_linear_group_covariateP,lty=4,col=colors_plot["MANOVA_random_linear_group_covariateP_col"],type = "b",pch = 1,lwd = 0.6)

lines(FigurexD$nseq,FigurexD$LM_lin_P_lin,lty=5,col=colors_plot["LM_lin_P_lin_col"],type = "b",pch = 1,lwd = 0.6)
lines(FigurexD$nseq,FigurexD$LM_lin_group_P_lin,lty=6,col=colors_plot["LM_lin_group_P_lin_col"],type = "b",pch = 1,lwd = 0.6)

#text(x=xmin , y=textn,labels=list_type[[i]][4],pos=4,xpd=NA)
...
plot(FigurexD$nseq,FigurexD$Watson_two_sample,col=colors_plot["Rayleigh_col"],type = 'b',lty=1,pch = 1,bty="n" ,
     ylab = ylab[12],xlab="Sample size per treat. comb.",xlim = c(xmin,xmax),ylim = c(0,ymax),lwd = 0.6,xpd=NA)
lines(FigurexD$nseq,FigurexD$G.test,col=colors_plot["HR_col"],lty=2,type = "b",pch = 1,lwd = 0.6,)

lines(FigurexD$nseq,FigurexD$MANOVA_linear_group_groupP,col=colors_plot["MANOVA_linear_group_groupP_col"],lty=3,type = "b",pch = 1,lwd = 0.6,)

lines(FigurexD$nseq,FigurexD$MANOVA_random_linear_group_groupP,col=colors_plot["MANOVA_random_linear_group_groupP_col"],lty=4,type = "b",pch = 1,lwd = 0.6,)

lines(FigurexD$nseq,FigurexD$LM_lin_group_P_group,col=colors_plot["LM_lin_group_P_group_col"],lty=5,type = "b",pch = 1,lwd = 0.6,)
