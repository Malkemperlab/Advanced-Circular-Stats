#Simulates hypothetical animal study with year and group effect (written by Lukas Landler, 08/Nov/2022)
...
m180_90=c(seq(dir,dir+rad(90),length.out = 5),rev(seq(dir,dir+rad(90),length.out = 5)))
m180_0=c(seq(dir,dir+rad(180),length.out = 5),rev(seq(dir,dir+rad(180),length.out = 5)))

m_list_1=list(mn,mn,m180_90,m180_0,m180_90,m180_0)
...
k_inter_less<-c(k_less_clustered,rev(k_less_clustered))
k_inter_more<-c(k_more_clustered,rev(k_more_clustered))
#k0<-rep(0,10)
k1<-rep(1,10)
k_list_1=list(k_inter_less,k_inter_more,k1,k1,k_inter_less,k_inter_less)
...
#plist (list with c(5) )
param_vm<-c("param_vm1","param_vm2","param_vm3","param_vm4","param_vm5",
            "param_vm6","param_vm7","param_vm8","param_vm9","param_vm10")
...
Name=c("Hypex_k_Int_less","Hypex_k_Int_more",
       "Hypex_dir_Intt_0_90","Hypex_dir_Int_0_180",
       "Hypex_k_Int_less_0_90","Hypex__k_Int_less_0_180")
...
random_MANOVA_F_lin_group_cov_group <- sapply(c(1:rans),function(x) random_MANOVA_lin_group[[x]][["stats"]][3,3])
MANOVA_F_lin_group_cov_group <- sapply(c(1:rans),function(x) MANOVA_approach_lin_group[[x]][["stats"]][3,3])
random_MANOVA_P_lin_group_cov_group<-vapply(1:rans,function(x) sum(1*(MANOVA_F_lin_group_cov_group[x]< random_MANOVA_F_lin_group_cov_group))/rans, FUN.VALUE = numeric(1))


#MANOVA approach added linear, grouping factor and interaction 
funManCirc_lin_group_int<-function(x) summary(manova(cbind(cos(x),sin(x)) ~ year_cov*group_fact),intercept=T)
MANOVA_approach_lin_group_int <- apply(Distribution,2, FUN=funManCirc_lin_group_int)
MANOVA_P_lin_group_int <- sapply(c(1:rans),function(x) MANOVA_approach_lin_group_int[[x]][["stats"]][1,6])
MANOVA_P_lin_group_cov_int <- sapply(c(1:rans),function(x) MANOVA_approach_lin_group_int[[x]][["stats"]][2,6])
MANOVA_P_lin_group_cov_group_int <- sapply(c(1:rans),function(x) MANOVA_approach_lin_group_int[[x]][["stats"]][3,6])
MANOVA_P_lin_group_cov_group_int_int <- sapply(c(1:rans),function(x) MANOVA_approach_lin_group_int[[x]][["stats"]][4,6])

#MANOVA approach Random used MANOVA_P from above
random_MANOVA_lin_group_int<-apply(random_sample,2,function(x) funManCirc_lin_group_int(x))

random_MANOVA_F <- sapply(c(1:rans),function(x) random_MANOVA_lin_group_int[[x]][["stats"]][1,3])
MANOVA_F<- sapply(c(1:rans),function(x) MANOVA_approach_lin_group_int[[x]][["stats"]][1,3])
random_MANOVA_P_lin_group_int<-vapply(1:rans,function(x) sum(1*(MANOVA_F[x]< random_MANOVA_F))/rans, FUN.VALUE = numeric(1))

random_MANOVA_F<- sapply(c(1:rans),function(x) random_MANOVA_lin_group_int[[x]][["stats"]][2,3])
MANOVA_F <- sapply(c(1:rans),function(x) MANOVA_approach_lin_group_int[[x]][["stats"]][2,3])
random_MANOVA_P_lin_group_cov_int<-vapply(1:rans,function(x) sum(1*(MANOVA_F[x]< random_MANOVA_F))/rans, FUN.VALUE = numeric(1))

random_MANOVA_F <- sapply(c(1:rans),function(x) random_MANOVA_lin_group_int[[x]][["stats"]][3,3])
MANOVA_F <- sapply(c(1:rans),function(x) MANOVA_approach_lin_group_int[[x]][["stats"]][3,3])
random_MANOVA_P_lin_group_cov_group_int<-vapply(1:rans,function(x) sum(1*(MANOVA_F[x]< random_MANOVA_F))/rans, FUN.VALUE = numeric(1))

random_MANOVA_F <- sapply(c(1:rans),function(x) random_MANOVA_lin_group_int[[x]][["stats"]][4,3])
MANOVA_F <- sapply(c(1:rans),function(x) MANOVA_approach_lin_group_int[[x]][["stats"]][4,3])
random_MANOVA_P_lin_group_cov_group_int_int<-vapply(1:rans,function(x) sum(1*(MANOVA_F[x]< random_MANOVA_F))/rans, FUN.VALUE = numeric(1))
...
funLimCirc2<-function(x) summary(lm(value[,x] ~ variable+year_2*group_2))
LM_approach <- lapply(c(1:rans),FUN=funLimCirc2)
LM_lin_group_int_P_intercept <- sapply(c(1:rans),function(x) LM_approach[[x]][["coefficients"]][1,4])
LM_lin_group_int_P_lin <- sapply(c(1:rans),function(x) LM_approach[[x]][["coefficients"]][3,4])
LM_lin_group_int_P_group <- sapply(c(1:rans),function(x) LM_approach[[x]][["coefficients"]][4,4])
LM_lin_group_int_P_group_int <- sapply(c(1:rans),function(x) LM_approach[[x]][["coefficients"]][5,4])
...
MANOVA_lin_group_int <- sum(1*(MANOVA_P_lin_group_int< 0.05))/rans
random_MANOVA_lin_group_int<-sum(1*(random_MANOVA_P_lin_group_int< 0.05))/rans

MANOVA_lin_group_cov_int<-sum(1*(MANOVA_P_lin_group_cov_int< 0.05))/rans
random_MANOVA_lin_group_cov_int<-sum(1*(random_MANOVA_P_lin_group_cov_int< 0.05))/rans

MANOVA_lin_group_cov_group_int<-sum(1*(MANOVA_P_lin_group_cov_group_int< 0.05))/rans
random_MANOVA_lin_group_cov_group_int<-sum(1*(random_MANOVA_P_lin_group_cov_group_int< 0.05))/rans

MANOVA_lin_group_cov_group_int_int<-sum(1*(MANOVA_P_lin_group_cov_group_int_int< 0.05))/rans
random_MANOVA_lin_group_cov_group_int_int<-sum(1*(random_MANOVA_P_lin_group_cov_group_int_int< 0.05))/rans
...
LM_lin_group_int_P_intercept_ <- sum(1*(LM_lin_group_int_P_intercept < 0.05))/rans
LM_lin_group_int_P_lin_ <- sum(1*(LM_lin_group_int_P_lin < 0.05))/rans
LM_lin_group_int_P_group_ <- sum(1*(LM_lin_group_int_P_group < 0.05))/rans
LM_lin_group_int_P_group_int_<-sum(1*(LM_lin_group_int_P_group_int < 0.05))/rans
...
names_names<-c("Rayleigh","HR","Model_comp","Watson_two_sample","G-test",
               ...
               "MANOVA_linear_group_int_interceptP","MANOVA_random_linear_group_int_interceptP",
               "MANOVA_linear_group_int_covariateP","MANOVA_random_linear_group_int_covariateP",
               "MANOVA_linear_group_int_groupP","MANOVA_random_linear_group_int_groupP",
               "MANOVA_linear_group_int_intP","MANOVA_random_linear_group_int_intP",
                "LM_P_intercept",
               "LM_lin_P_intercept","LM_lin_P_lin",
               "LM_lin_group_P_intercept", "LM_lin_group_P_lin","LM_lin_group_P_group", 
               "LM_lin_group_int_P_intercept", "LM_lin_group_int_P_lin","LM_lin_group_int_P_group", "LM_lin_group_int_P_int")
...
Power[1,]<-c(Rayleigh,HR,AIC2.Z,WatsonTwoSamp,G.test,
             ...
             MANOVA_lin_group_int,random_MANOVA_lin_group_int,
             MANOVA_lin_group_cov_int,random_MANOVA_lin_group_cov_int,
             MANOVA_lin_group_cov_group_int,random_MANOVA_lin_group_cov_group_int,
             MANOVA_lin_group_cov_group_int_int,random_MANOVA_lin_group_cov_group_int_int,
             LM_P_intercept_,LM_lin_P_intercept_,
             LM_lin_P_lin_, LM_lin_group_P_intercept_,
             LM_lin_group_P_lin_,LM_lin_group_P_group_,
             LM_lin_group_int_P_intercept_,LM_lin_group_int_P_lin_,LM_lin_group_int_P_group_,LM_lin_group_int_P_group_int_)
...
text <- paste0(Name[i],"_n_",min(nseq),"-",max(nseq),"_its_",rans,"_Dist_",
               plist,"_",distt, "_m=",min(deg(m1)),"-",max(deg(m1)), "_conc=",min(k1),"-",max(k1),".csv")[1]

text2 <- paste0("ExampleDist_",Name[i],"_n_",min(nseq),"-",max(nseq),"_its_",rans,"_Dist_",
                plist,"_",distt, "_m=",min(deg(m1)),"-",max(deg(m1)), "_conc=",min(k1),"-",max(k1),".RDS")[1]
